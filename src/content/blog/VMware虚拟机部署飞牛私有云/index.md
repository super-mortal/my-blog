...
slug: 'vmware-xu-ni-ji-bu-shu-fnos-fei-niu-si-you-yun'
tags:
  - 飞牛nas
  - 总结
language: '中文'
...
![VMware Workstation 中已创建好的 fnOS 虚拟机：内存 4GB、2 核 CPU、SCSI 硬盘 20GB、CD/DVD 挂载 fnOS ISO、网络适配器 NAT，配置文件位于 D:\always\Debian\fnOS.vmx，工作站版本 17.5.x，当前状态为已关机](1.png)

8. 然后选中fnOS（自己填写的名字），点击开启此虚拟机

![fnOS 飞牛私有云 V1.1.26 安装向导第一步「选择安装位置」：识别到 [sda] VMware Virtual S 这块 20GB 虚拟 HDD、剩余 20GB 可用，点击下一步继续](2.png)
...
![浏览器打开 http://192.168.147.130:5666/system 看到的 fnOS「开始使用 fnOS」初始化表单：填写设备名称、超级管理员账号、密码、确认密码并勾选隐私协议与服务条款后点击「进入 fnOS」](3.png)
...
![fnOS Web 控制台「系统设置 - 创建存储空间」向导第 1 步（共 4 步）：选择文件系统，对比 Btrfs（推荐，支持快照）、ZFS（支持快照/压缩/去重，适合企业）、ext4（不支持快照/压缩/数据校验，适合轻量文件服务），点击下一步继续](4.png)
...
![fnOS Docker 应用首次启动后的概览页：Docker 服务已开启、存储位置指向「存储空间 1 Basic」并启用开机自动启动，当前无容器、无项目、无镜像运行，下方实时显示 CPU、内存与网络使用率](5.png)

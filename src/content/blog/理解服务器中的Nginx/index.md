...
![手绘风示意图「没有 Nginx 的世界 - 网站访问直连后端的崩溃日常」：卡通用户拿着 HTTP Request 直接访问一台叫 Hachiware 的后端服务器，服务器冒汗过热、链接断裂崩溃，说明没有 Nginx 作为前置代理时用户请求直冲后端会造成服务不稳和安全隐患](meiyounginx.png)
...
![手绘风示意图「网站访问全过程」：用户浏览器访问 example.com → DNS 服务器像电话簿把域名翻译成 IP 1.2.3.4 → HTTPS 加密安全通道连接到 1.2.3.4:443 → Nginx 高效前台管家接收并分发请求 → 后端程序厨师处理业务、读写数据库 → 页面返回浏览器展示，直观说明 Nginx 在服务器架构中承担反向代理与流量入口的角色](nginxzuoyong.png)

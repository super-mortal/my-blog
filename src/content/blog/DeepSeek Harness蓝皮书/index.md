...
先交代下背景：我从 dsh 刚出来就在折腾 [DeepSeek Harness](https://github.com/super-mortal/DeepSeekHarnessGuide)（简称 dsh），一路踩了不少坑。想找份"从入门到生产"的完整教程，结果翻了一圈官方仓库——只有 API 文档，没有一篇能照着跑的实战指南。行吧，没人写，那我写。于是就有了这份 **[DeepSeek Harness 蓝皮书](https://dsh.supermortal.top/)**（[GitHub 仓库](https://github.com/super-mortal/DeepSeekHarnessGuide)）。
...
简单来说，这是一份**以真实任务为主线**的 dsh 学习指南：从跑通第一个能验收的 Agent 开始，到搭出一套可复用的工作系统。它不是 API 文档的翻译——官方文档讲"是什么"，我讲"怎么跑通"。每一步的命令、截图、甚至报错，都是真实操作记录下来的，**照着做就能复现**。

视觉上我搞了一套 **E-INK 纸感 × DeepSeek 墨蓝** 主题：米白纸底、墨色文字、荧光黄标注，动效几乎为零，读起来更像一本纸书而不是网页。支持中/英/日/韩/西/葡 **6 种语言**，还内置了自研图片灯箱（点击放大、Ctrl+滚轮缩放、拖拽平移）。
...
如果你也在用 dsh，或者对 Agent 工作流感兴趣，直接在仓库按 PART 目录读就行。觉得有用的话，点个 Star 就是最大的鼓励 ⭐
